...
public class SGEOutputSearcher {
    ...
    /* used to parse output from sge command to determine if the sge command was found */
    private String commandNotFoundRegexp;

    /* used to parse output if invalid sge command was used incorrectly */
    private String invalidUsageRegexp;
	...
	public void setCommandNotFoundRegexp(String commandNotFoundRegexp) {
		this.commandNotFoundRegexp = commandNotFoundRegexp;
	}
	
	public void setInvalidUsageRegexp(String invalidUsageRegexp) {
		this.invalidUsageRegexp = invalidUsageRegexp;
	}
     ...
     * @return true if there is a match, false otherwise.
     */
    public boolean matchCommandNotFound(String input) {
        Preconditions.checkArgument(commandNotFoundRegexp != null, "command not found regexp for delete command must have been injected by this point");
        Preconditions.checkArgument(input != null, "the input cannot be null");

        return hasMatch(commandNotFoundRegexp, input);
    }
     ...
     * @return true if there is a match, false otherwise.
     */
    public boolean matchInvalidUsage(String input) {
        Preconditions.checkArgument(invalidUsageRegexp != null, "invalid usage regexp for delete command must have been injected by this point");
        Preconditions.checkArgument(input != null, "the input cannot be null");

        return hasMatch(invalidUsageRegexp, input);
    }
    ...
    private void scan(Pattern pattern, String input, SGEOutput output) {
        ...
        while (scanner.hasNext()) {
            ...
            if (matcher.matches()) {
                output.put(matcher.group(1).trim(), matcher.group(2).trim());
            } else {
	...
	public String getCommandNotFoundRegexp() {
		return commandNotFoundRegexp;
	}
	
	
	public String getInvalidUsageRegexp() {
		return invalidUsageRegexp;
	}
}
