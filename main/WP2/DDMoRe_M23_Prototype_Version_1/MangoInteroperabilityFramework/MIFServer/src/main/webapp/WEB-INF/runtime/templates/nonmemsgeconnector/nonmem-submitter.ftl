...
<#if "NMFE" = job.executionRequest.getType()>
    <#assign nmOutput = "${job.getRunLstFileName()}">
</#if>

<#if "PsN_EXEC" = job.executionRequest.getType()>
    <#assign psnNmOutput = "-outputfile='${job.getRunLstFileName()}'">
</#if>
...
${executable} ${job.executionRequest.executionParameters!} ${psnNmOutput!} "${job.getExecutionFileName()}" ${nmOutput!}
ext=$?
