$PROBLEM    System Test 1
$INPUT      SID SEX AGE RACE HT SMOK HCTZ PROP CON AMT WT TIME SECR DV
            DROP=RATE EVID SS II ID OCC
$DATA       Data1 IGNORE=@
$SUBROUTINE ADVAN2 TRANS2
$OMEGA  .128
 .142
$OMEGA  1.82
$PK
 
    TVCL=THETA(1)
    TVV=THETA(2)
    TVKA=THETA(3)
    CL=TVCL*EXP(ETA(1))
    V =TVV *EXP(ETA(2))
    KA=TVKA*EXP(ETA(3))
    S2=V
$ERROR
 
    IPRED = F
    IRES  = DV - F
    W     = F
    IF(W.EQ.0) W = 1
    IWRES = IRES/W
    Y     = IPRED + W*EPS(1)

$THETA  18.7
 87.3
 2.13
$SIGMA  0.0231
$ESTIMATION NOABORT POSTHOC
$TABLE      ID TIME IPRED IWRES NOPRINT ONEHEADER FILE=sdtab1
$TABLE      ID CL V KA NOPRINT ONEHEADER FILE=patab1
$TABLE      ID AGE HT WT SECR NOPRINT ONEHEADER FILE=cotab1
$TABLE      ID SEX RACE SMOK HCTZ PROP CON NOPRINT ONEHEADER
            FILE=catab1
$TABLE      ID OCC TIME IPRED IWRES NOPRINT ONEHEADER FILE=mutab1
$TABLE      SID NOPRINT ONEHEADER FILE=mytab1

