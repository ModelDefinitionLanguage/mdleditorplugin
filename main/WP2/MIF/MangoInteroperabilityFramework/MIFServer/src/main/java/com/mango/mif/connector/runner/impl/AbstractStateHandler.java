...
public abstract class AbstractStateHandler implements StateHandler {
    ...
    protected void fail() {
    	if(DriverProcessingStatus.CANCELLED.equals(driver.getProcessingStatus())) {
    		/* if the job has been cancelled, don't handle failure, 
    		* because cancellation might result in sate handlers processing failures
    		*/ 
    		return;
    	}
        StringBuilder builder = new StringBuilder();
        builder.append("Processing of a handler for state " + handledState + " failed. ");
        if(stateHandlerException!=null) {
            LOG.error(builder.toString(),stateHandlerException);
        } else {
