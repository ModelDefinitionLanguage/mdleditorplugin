...
public class PredStatementTest {
	...
	private final String prolactinModel = PATH + "Friberg2009Prolactin_v20140523v14_NONMEM.xml"
	
	@Test
	public void testSimpleAssignedParameters() {
        File src = new File(Thread.currentThread().getContextClassLoader().getResource(prolactinModel).getPath());
...
ELSE
IF(((${PREFIX}PAT.EQ.${PREFIX}PAT).AND.(${PREFIX}SEX.EQ.${PREFIX}M)).AND.(${PREFIX}STUDY.NE.${PREFIX}STU101))
	${PREFIX}POP_PRL0=THETA(2)
ELSE
IF(((${PREFIX}PAT.EQ.${PREFIX}PAT).AND.(${PREFIX}SEX.EQ.${PREFIX}F)).AND.(${PREFIX}STUDY.NE.${PREFIX}STU101))
	${PREFIX}POP_PRL0=THETA(3)
ELSE
IF(((${PREFIX}PAT.EQ.${PREFIX}PAT).AND.(${PREFIX}SEX.EQ.${PREFIX}M)).AND.(${PREFIX}STUDY.EQ.${PREFIX}STU101))
	${PREFIX}POP_PRL0=THETA(4)
ELSE
IF(((${PREFIX}PAT.EQ.${PREFIX}PAT).AND.(${PREFIX}SEX.EQ.${PREFIX}F)).AND.(${PREFIX}STUDY.EQ.${PREFIX}STU101))
