...
public class MdlToJsonModelsTest {
    ...
    public void testMdlToJsonConversion() throws IOException {
        final ConverterRunner runner = new ConverterRunner(
            this.model, OUTPUT_FILE_EXTENSION, "MDL", MDL_VERSION, "JSON", JSON_VERSION,
    ...
    private static class ConverterJsonOutputFailureCheckerImpl extends ConverterOutputFailureCheckerImpl {
        ...
        public void check(File expectedOutputFile, File stdoutFile, File stderrFile) {
            ...
            try {
            	...
            	assertNotNull("Checking that the JSON from file " + expectedOutputFile + " was valid JSON i.e. was able to be parsed successfully", jsonAsTree);
            	MdlToJsonModelsTest.LOGGER.debug(jsonAsTree);
            } catch (Exception e) {
            	MdlToJsonModelsTest.LOGGER.error("Exception \"" + e.getMessage() + "\" thrown parsing JSON from file " + expectedOutputFile, e);
