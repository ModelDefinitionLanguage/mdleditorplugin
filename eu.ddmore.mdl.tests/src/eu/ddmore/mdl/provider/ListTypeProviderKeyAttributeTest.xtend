...
class ListTypeProviderKeyAttributeTest {
	...
	val static String KEY_ATT1 = "keyAtt1"
	val static String KEY_ATT2 = "keyAtt2"
	...
	var TypeDefinition intType
	var TypeDefinition stringType
	...
	def void setUp(){
		...
		intType = libDefn.createTypeDefinition('Int', TypeClass.INT)
		keyAttDefn = createAttributeDefn(KEY_ATT1, createTypeSpec(intType, null))
		list1Att2Defn = createAttributeDefn(LIST1_ATT2, createTypeSpec(realType, null))
		listDefn1 = libDefn.createListTypeDefinition(createTypeSpec(realType, null), null, #[ keyAttDefn, list1Att2Defn ], #[true, true])

		stringType = libDefn.createTypeDefinition('Str', TypeClass.STRING)
		list2KeyAttDefn = createAttributeDefn(KEY_ATT2, createTypeSpec(stringType, null))
		...
		blkDefn = libDefn.createBlockDefnWithKeyAttributes("testBlk", #{ keyAttDefn->listDefn1, list2KeyAttDefn->listDefn2 })
...
//		testInstance = null
	...
	def void testGetListDefinitionWithDifferentKeysList(){
		val ld1 = obsBlk.createListDefn("CL1", #[
			createAssignPair(KEY_ATT1, createIntLiteral(1)),
			createAssignPair(LIST1_ATT2, createRealLiteral(2.2))
		])
		val ld2 = obsBlk.createListDefn("CL2", #[
			createAssignPair(KEY_ATT2, createStringLiteral("Str")),
	...
	@Test
	def void testGetListDefinitionWithWrongKeyAtt(){
		val ld = obsBlk.createListDefn("CL", #[
			createAssignPair(WRONG_KEY_ATT, createRealLiteral(2.1)),
			createAssignPair(LIST1_ATT2, createRealLiteral(2.2))
		])
	...
	def void testGetAttributeDefinitionKey(){
		...
		val actualAtTDefn = ldi.getAttributeDefinition(KEY_ATT1)
	...
	def void testGetAttributeType(){
		val ldi = new ListDefInfo(listDefn1)
		val actualType = ldi.getAttributeType(KEY_ATT1)
		assertEquals("expected defn", intType.typeInfo, actualType)
	}
	...
	@Test
	def void testGetAttributeTypeValuePairWrongKeyAttribute(){
		val vp = createAssignPair(LIST1_ATT2, createRealLiteral(2.2)) 
		obsBlk.createListDefn("CL", #[
			createAssignPair(WRONG_KEY_ATT, createIntLiteral(3)),
			vp
		])
	...
	def void testGetAttributeTypeValuePairMissingKeyAtt(){
		...
		obsBlk.createListDefn("CL", #[
			createEnumPair(LIST1_ATT2, "val2"),
			vp
		])
	...
	def void testGetUnusedMandatoryAttributesNoneMissingWithOptional(){
		val ld = obsBlk.createListDefn("CL", #[
			createAssignPair(KEY_ATT2, createIntLiteral(1))
		])
	...
	@Test
	def void testIsKeyAttributeDefinedWithKey(){
		val ld = obsBlk.createListDefn("CL", #[
			createAssignPair(KEY_ATT1, createIntLiteral(1))
		])
	...
	@Test
	def void testIsKeyAttributeDefinedOtherKey(){
		val ld = obsBlk.createListDefn("CL", #[
			createAssignPair(KEY_ATT2, createIntLiteral(1))
		])
	...
	@Test
	def void testIsKeyAttributeDefinedNonKeyAtt(){
		val ld = obsBlk.createListDefn("CL", #[
